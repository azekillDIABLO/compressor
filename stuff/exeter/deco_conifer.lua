...
newnode = exeter.clone_node("default:pine_needles")
if exeter.noleafdecay then
	newnode.groups.leafdecay = 0
end
newnode.tiles = {"default_pine_needles.png^[colorize:#FF0000:20"}
minetest.register_node("exeter:pine_needles2", newnode)
newnode.tiles = {"default_pine_needles.png^[colorize:#FFFF00:20"}
minetest.register_node("exeter:pine_needles3", newnode)
newnode.tiles = {"default_pine_needles.png^[colorize:#00FF00:20"}
minetest.register_node("exeter:pine_needles4", newnode)

if exeter.glow then
	minetest.register_node("exeter:pine_tree_glowing_moss", {
...
function exeter.generate_conifer_schematic(trunk_height, radius, trunk, leaf)
	...
	for z = -radius,radius do
		for y = 1,trunk_top do
			...
			for x = -radius,radius do
				...
				if x == 0 and z == 0 then
					if trunk == "default:pine_tree" and exeter.glow and math.random(1,10) == 1 then
						s.data[i].name = "exeter:pine_tree_glowing_moss"
					else
						s.data[i].name = trunk
					end
					s.data[i].param1 = 255
...
function exeter.generate_default_conifer_schematic(trunk, leaf)
	local height = 13 + 1
	local width = 5
	local s = exeter.schematic_array(width, height, width)
...
-- generic conifers
exeter.schematics.conifer_trees = {}
leaves = {"default:pine_needles", "exeter:pine_needles2", "exeter:pine_needles3", "exeter:pine_needles4"}
for i = 1,#leaves do
	local max_r = 4
	for r = 2,max_r do
		local schem = exeter.generate_conifer_schematic(2, r, "default:pine_tree", leaves[i])

		exeter.schematics.conifer_trees[#exeter.schematics.conifer_trees+1] = schem
...
if false then
	...
	leaves = {"default:pine_needles", "exeter:pine_needles2", "exeter:pine_needles3", "exeter:pine_needles4"}
	for i = 1,#leaves do
		local schem = exeter.generate_default_conifer_schematic("default:pine_tree", leaves[i])
...
function default.grow_new_pine_tree(pos, bad)
	local schem = exeter.schematics.conifer_trees[math.random(1,#exeter.schematics.conifer_trees)]
