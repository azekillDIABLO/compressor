...
local newnode = exeter.clone_node("default:leaves")
if exeter.noleafdecay then
	newnode.groups.leafdecay = 0
end
newnode.tiles = {"default_leaves.png^[colorize:#FF0000:20"}
minetest.register_node("exeter:leaves2", newnode)
newnode.tiles = {"default_leaves.png^[colorize:#FFFF00:20"}
minetest.register_node("exeter:leaves3", newnode)
newnode.tiles = {"default_leaves.png^[colorize:#00FFFF:20"}
minetest.register_node("exeter:leaves4", newnode)
newnode.tiles = {"default_leaves.png^[colorize:#00FF00:20"}
minetest.register_node("exeter:leaves5", newnode)

if exeter.glow then
	minetest.register_node("exeter:tree_glowing_moss", {
...
function exeter.generate_tree_schematic(trunk_height, radii, trunk, leaf, fruit, limbs)
	...
	local s = exeter.schematic_array(width, height, width)

	-- the main trunk
	for y = 1,trunk_top do
		local i = radii.z*width*height + y*width + radii.x + 1
		if trunk == "default:tree" and exeter.glow and math.random(1,10) == 1 then
			s.data[i].name = "exeter:tree_glowing_moss"
		else
			s.data[i].name = trunk
		end
		s.data[i].param1 = 255
	...
	exeter.generate_leaves(s, leaf, {x=0, y=trunk_top, z=0}, radii.x, fruit)
	...
	if radii.x > 3 and limbs then
		for _, p in pairs(limbs) do
			...
			s.data[i].param1 = 255
			s.data[i].force_place = false
			exeter.generate_leaves(s, leaf, p, radii.x, fruit, true)
		end
				...
				for x = -radii.x,radii.x do
					...
					if x^2/(radii.x-3)^2 + y^2/(radii.y-3)^2 + z^2/(radii.z-3)^2 <= 1 then
						if math.random(1,6) == 1 then
							...
							s.data[i].param1 = 255
							s.data[i].force_place = false
							exeter.generate_leaves(s, leaf, {x=x, y=trunk_top+y, z=z}, radii.x, fruit, true)
						end
					end
				end
			end
...
function exeter.generate_leaves(s, leaf, pos, radius, fruit, adjust)
...
exeter.schematics.deciduous_trees = {}
local leaves = {"default:leaves", "exeter:leaves2", "exeter:leaves3", "exeter:leaves4", "exeter:leaves5"}
for i = 1,#leaves do
	local max_r = 6
	local fruit = nil
	...
	for r = 3,max_r do
		local schem = exeter.generate_tree_schematic(2, {x=r, y=r, z=r}, "default:tree", leaves[i], fruit)

		exeter.schematics.deciduous_trees[#exeter.schematics.deciduous_trees+1] = schem
...
function default.grow_new_apple_tree(pos, bad)
	local schem = exeter.schematics.deciduous_trees[math.random(1,#exeter.schematics.deciduous_trees)]
