...
newnode = exeter.clone_node("default:jungleleaves")
newnode.tiles = {"default_jungleleaves.png^[colorize:#FF0000:10"}
minetest.register_node("exeter:jungleleaves2", newnode)
newnode.tiles = {"default_jungleleaves.png^[colorize:#FFFF00:30"}
minetest.register_node("exeter:jungleleaves3", newnode)


-- Create a schematic for a jungle tree.
function exeter.generate_jungle_tree_schematic(trunk_height, trunk, leaf)
	...
	for y = 1,trunk_top+2 do
		if y > trunk_height and (y == trunk_top or math.random(1,height - y) == 1) then
			...
			for j = -1,1,2 do
				...
				s.data[i].param1 = 255
				s.data[i].force_place = true
				exeter.generate_canopy(s, leaf, {x=j*x, y=y, z=j*z})
			end
		end
	end
...
function exeter.generate_canopy(s, leaf, pos)
...
exeter.schematics.jungle_trees = {}
leaves = {"default:jungleleaves", "exeter:jungleleaves2", "exeter:jungleleaves3"}
for i = 1,#leaves do
	local max_h = 7
	for h = 5,max_h do
		local schem = exeter.generate_jungle_tree_schematic(h*2, "default:jungletree", leaves[i])

		exeter.schematics.jungle_trees[#exeter.schematics.jungle_trees+1] = schem
...
function default.grow_new_jungle_tree(pos, bad)
	local schem = exeter.schematics.jungle_trees[math.random(1,#exeter.schematics.jungle_trees)]
